# 👋 E ai, belezinha? Meu nome é Jonatan Marques

**Desenvolvedor Full Stack | Especialista em Front-End**

📍 Rio de Janeiro - Brasil  
💼 Desenvolvendo soluções web modernas, escaláveis e focadas em performance  
📧 Contato: marques.jonatan@hotmail.com  
🔗 LinkedIn / GitHub: **JonatanMds**

---

## 🚀 Sobre mim

Sou Desenvolvedor Full Stack com visão completa do ciclo de vida do software.  
Tenho sólida experiência em **React, Next.js e TypeScript**, atuando fortemente no Front-End, além de conhecimento em **arquitetura de sistemas, APIs REST e automação com Python**.

Atuo frequentemente em:
- Planejamento e arquitetura de projetos
- Otimização de performance e acessibilidade
- Code review e mentoria de desenvolvedores
- Deploy e manutenção de aplicações em produção

Gosto de ambientes dinâmicos, produtos reais e times que valorizam qualidade técnica.

---

## 🧠 Stack Principal

**Front-End**
- React.js • Next.js • TypeScript • JavaScript  
- Tailwind CSS • UI/UX • Responsive Design • Figma

**Back-End**
- Node.js • APIs REST • Python (automação)

**Qualidade & Metodologia**
- Testes unitários • Performance Web • Code Review  
- Scrum • Metodologias Ágeis

**Ferramentas**
- Git • Azure

---

## 🧰 Tech Stack

### 🎨 Frontend
![JavaScript](https://img.shields.io/badge/JavaScript-000?style=for-the-badge&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-000?style=for-the-badge&logo=typescript)
![React](https://img.shields.io/badge/React-000?style=for-the-badge&logo=react)
![Next.js](https://img.shields.io/badge/Next.js-000?style=for-the-badge&logo=nextdotjs)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-000?style=for-the-badge&logo=tailwindcss)
![Styled Components](https://img.shields.io/badge/Styled--Components-000?style=for-the-badge&logo=styled-components)

### 🧠 Backend
![Node.js](https://img.shields.io/badge/Node.js-000?style=for-the-badge&logo=node.js)
![.NET](https://img.shields.io/badge/.NET-000?style=for-the-badge&logo=dotnet)
![C#](https://img.shields.io/badge/C%23-000?style=for-the-badge&logo=csharp)
![Prisma](https://img.shields.io/badge/Prisma-000?style=for-the-badge&logo=prisma)
![MySQL](https://img.shields.io/badge/MySQL-000?style=for-the-badge&logo=mysql)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-000?style=for-the-badge&logo=postgresql)

### ⚙️ DevOps & Tools
![Docker](https://img.shields.io/badge/Docker-000?style=for-the-badge&logo=docker)
![Vercel](https://img.shields.io/badge/Vercel-000?style=for-the-badge&logo=vercel)
![AWS](https://img.shields.io/badge/AWS-000?style=for-the-badge&logo=amazonaws)
![Git](https://img.shields.io/badge/Git-000?style=for-the-badge&logo=git)
![Insomnia](https://img.shields.io/badge/Insomnia-000?style=for-the-badge&logo=insomnia)
![Figma](https://img.shields.io/badge/Figma-000?style=for-the-badge&logo=figma)

---

## 🌱 Além do código

Nas horas vagas, estou sempre criando e experimentando **novos produtos SaaS**, estudando arquitetura, performance e modelos de negócio digitais.

Sou apaixonado por **Esportes(Futebol / Basquete / NFL) e video games**, o que reforça meu interesse por tecnologia, performance, trabalho em equipe e tomada de decisão sob pressão — características que levo para o ambiente profissional.

Acredito que bons produtos nascem da combinação entre **engenharia sólida, visão de negócio e paixão pelo que se constrói**.

---

## 📌 O que você encontrará aqui

Neste GitHub você vai encontrar:
- Projetos com **React, Next.js e TypeScript**
- Boas práticas de arquitetura Front-End
- Código limpo, organizado e escalável
- Experimentos com performance, UX e automação

---

## 🤝 Vamos conversar?

Se você está buscando um desenvolvedor focado em qualidade, performance e entrega de produto real:

📧 **marques.jonatan@hotmail.com**  
💼 **LinkedIn/GitHub: JonatanMds**

Obrigado por visitar meu perfil! 🚀
